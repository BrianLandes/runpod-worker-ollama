...
# Kill any existing ollama processes (-r: skip kill when pgrep finds nothing)
pgrep ollama | xargs -r kill
...
# Models are baked into the image at build time (see Dockerfile) — no runtime pull.
